...
```{r reproduce_bivar_fig, echo=TRUE}
sig_x <- 1
sig_y <- 1
num_obs <- 4000
rho <- 0.5
...
```{r benchmark_updating, echo=TRUE}
...
```{r benchmark_quantile_est, echo=TRUE}
obs <- rnorm(1e6)
td <- tdigest(obs)
h_est <-
